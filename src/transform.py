...
if __name__ == "__main__":
    try:
        # Executa toda a esteira de transformação
        df_final = data_transformations()
        
        # Imprime uma amostra da tabela para visualização
        print("\n" + "="*50)
        print("🎯 AMOSTRA DOS DADOS PRONTOS PARA ANÁLISE")
        print("="*50)
        
        # Selecionando algumas colunas chave para não poluir a tela
        colunas_analise = ['city_name', 'temperature', 'datetime', 'weather_description']
        print(df_final[colunas_analise].to_string(index=False))
        
        print("\n" + "="*50)
        print("📊 INFORMAÇÕES DO DATAFRAME (Tipagem)")
        print("="*50)
        df_final.info()
